...
def pam_verify(pwd : str) -> tuple[bool, str]:
    ...
    cs = db.cursor()
    vals : tuple[tuple[str, str]]= cs.execute("SELECT hash, salt from keys WHERE is_master = 1").fetchall()
    # try:
...
def pam_add(service : str, username : str, pwd : str, key : str) -> None:
    ...
    cs = db.cursor()
    salt = secrets.token_bytes(32)
    dek = argon2.PasswordHasher().hash(password=key, salt=salt)
    cs.execute("INSERT INTO keys VALUES (?, ?, ?, ?, ?)", (service, username, dek, salt, 0))

    nonce = secrets.token_bytes(16)
    aes = AES.new(key=dek, mode=AES.MODE_GCM, nonce=nonce)
    enc = aes.encrypt(pwd)
